describe('Deliveries page', () => {
  ...
  it('should show a "Confirmed" modal when confirm cta is clicked', () => {
    ...
    cy.getByTestId('header-cta')
      .click()
      .then(() => {
        expect(stub.getCall(0).calledWith('Confirmed' as any)).eq(true);
      });

    cy.getByTestId('subheader-cta')
      .click()
      .then(() => {
        expect(stub.getCall(1).calledWith('Confirmed' as any)).eq(true);
      });
  });
  ...
  it('should show a "Rescheduled" modal when reschedule cta is clicked', () => {
    ...
    cy.getByTestId('header-cta')
      .click()
      .then(() => {
        expect(stub.getCall(0).calledWith('Rescheduled' as any)).eq(true);
      });

    cy.getByTestId('subheader-cta')
      .click()
      .then(() => {
        expect(stub.getCall(1).calledWith('Rescheduled' as any)).eq(true);
      });
  });
});
